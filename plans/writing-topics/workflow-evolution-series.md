...
3. **Design.** How design decisions happen when the agent can render options in parallel. Design explorers as a first-class workflow primitive (palette explorers, card deep-dives, diagram aesthetic explorers). Rationale lives inline in the artefact, not in Figma comments. The shift from "pick a direction then execute" to "render 5 directions side-by-side then pick." Feedback on concrete renders is far higher fidelity than feedback on descriptions. Candidate thesis: the design explorer is the process for any decision with more than two axes.
   ...
   **Hero example for this sub-post:** the-weekly evolution showcase — three live interactive versions of the same product (POC → design audit → tokenised) embedded under Lesson 2 of the case study as decision-led evidence that the design process changed. Bookmarked transcript: `the-weekly-evolution-showcase-decision`. Plan: `plans/the-weekly-evolution-showcase.md`. Why it's the hero: the conversation captures the meta-frame ("an assertion needs evidence, build the evidence") and the implementation detail that mattered most (iframe state preservation via `hidden`-toggling not `src`-swapping). Both are transferable lessons, not the-weekly-specific.

   **Process-diagram visual to build:** a side-by-side comparison of the design process I used for projects 1, 2, 3 — **the-weekly** (Project 1, no design phase, jumped concept → technical plan → build), **portfolio** (Project 2, design explorers as the primary primitive, parallel-render-then-pick, design system scaffolded as a workspace artefact), **planner** (Project 3, design explorers + Stitch MCP for layout ideation + 40-render batch with upfront gates). The diagram should make the *evolution* legible at a glance: each project gets its own column or row, with the same canonical phase axis (idea → spec → visual direction → component build → polish), and the cells either show the artefact used (none / explorer / Stitch render) or the failure mode at that phase (e.g. the-weekly: "skipped → biggest mistake"). Archetype is closest to a flowchart matrix; falls under the four-archetypes rule (`writing-style.md` Part 3 rule 7) as a flowchart-with-comparison-axis. Build it as part of the sub-post 3 drafting work. Earlier sessions' Stitch transcripts (chunk 4c.1, planner 40-render batch) should be bookmarked separately to feed the visual evidence for projects 2 and 3.
